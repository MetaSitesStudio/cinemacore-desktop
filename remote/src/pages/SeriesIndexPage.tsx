...
export const SeriesIndexPage: React.FC = () => {
  ...
  const filteredSeries = React.useMemo(() => {
    if (!selectedGenre) return data.series;
    return data.series.filter(s => s.genres?.some((g: any) => {
      const name = typeof g === 'string' ? g : g.name;
      return name === selectedGenre;
    }));
  }, [data.series, selectedGenre]);

  if (loading) return <div className="min-h-[50vh] flex items-center justify-center text-primary">Loading Series...</div>;
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        {filteredSeries.length === 0 && <p className="text-gray-500 text-sm col-span-full">No series found.</p>}
      </div>
    </div>
  );
};
